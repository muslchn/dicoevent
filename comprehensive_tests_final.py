...
class DicoEventAPITester:
    ...
    def test_user_endpoints(self):
        ...
        # Test 1: List Users with Super User (should work)
        response = self.make_request('GET', '/users/', headers=super_headers, expected_status=200)
        if response:
            users_data = response.json()
            self.log_test_result("List Users (Super User)", "PASS")
            
            # Find specific users for further testing
            users = users_data.get('results', []) if isinstance(users_data, dict) else users_data
            dicoding_user = next((u for u in users if u.get('username') == 'dicoding'), None)
            aras_user = next((u for u in users if u.get('username') == 'Aras'), None)
            
            if dicoding_user and aras_user:
                dicoding_user_id = dicoding_user.get('id')
                aras_user_id = aras_user.get('id')
                
                # Test getting own profile
                response = self.make_request('GET', f'/users/{dicoding_user_id}/', headers=regular_headers, expected_status=200)
                if response:
                    self.log_test_result("Get Own User Profile", "PASS")
                
                # Test regular user accessing super user profile (should fail)
                response = self.make_request('GET', f'/users/{aras_user_id}/', headers=regular_headers, expected_status=403)
                ...
                response = self.make_request('GET', f'/users/{dicoding_user_id}/', headers=super_headers, expected_status=200)
                if response:
                    self.log_test_result("Super User Access Any Profile", "PASS")
        
        # Test 2: Regular user trying to list users (should fail)
        response = self.make_request('GET', '/users/', headers=regular_headers, expected_status=403)
        if response is None:  # Expected failure
    ...
    def test_event_endpoints(self):
        ...
        # Test 1: List Events (should work with authentication)
        response = self.make_request('GET', '/events/', headers=regular_headers, expected_status=200)
        if response:
            self.log_test_result("List Events (Authenticated)", "PASS")
        
        # Test 2: Create Event with Super User
        event_data = {
        ...
        response = self.make_request('POST', '/events/', data=event_data, headers=super_headers, expected_status=201)
        if response:
            event_data_response = response.json()
            event_id = event_data_response.get('id')
            self.test_data['event_id'] = event_id
            self.log_test_result("Create Event (Super User)", "PASS")
        
        # Test 3: Get Specific Event
        if 'event_id' in self.test_data:
            response = self.make_request('GET', f'/events/{self.test_data["event_id"]}/', headers=regular_headers, expected_status=200)
            if response:
    ...
    def test_ticket_endpoints(self):
        ...
        if 'event_id' in self.test_data:
            ...
            response = self.make_request('POST', '/ticket-types/', data=ticket_data, headers=super_headers, expected_status=201)
            if response:
                ticket_data_response = response.json()
                ticket_type_id = ticket_data_response.get('id')
                self.test_data['ticket_type_id'] = ticket_type_id
                self.log_test_result("Create Ticket Type", "PASS")
            
            # Test 2: List Ticket Types
            response = self.make_request('GET', '/ticket-types/', headers=super_headers, expected_status=200)
            if response:
    ...
    def test_registration_endpoints(self):
        ...
        # Test 1: Create Registration with required attendee fields
        if 'event_id' in self.test_data and 'ticket_type_id' in self.test_data:
            ...
            response = self.make_request('POST', '/registrations/', data=reg_data, headers=regular_headers, expected_status=201)
            if response:
                reg_data_response = response.json()
                reg_id = reg_data_response.get('id')
                self.test_data['registration_id'] = reg_id
                self.log_test_result("Create Registration", "PASS")
            
            # Test 2: List Registrations
            response = self.make_request('GET', '/registrations/', headers=regular_headers, expected_status=200)
            if response:

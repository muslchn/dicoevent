...
class PaymentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Payment
        fields = [
            'id', 'registration', 'registration_detail', 'user', 'user_detail',
        ...
        read_only_fields = [
            'id', 'user_detail', 'registration_detail', 'amount', 'currency',
            'initiated_at', 'processed_at', 'completed_at', 'refunded_at'
        ]
    ...
    def validate(self, attrs):
        ...
        if registration:
            ...
            if hasattr(registration, 'payment'):
                raise serializers.ValidationError("Payment already exists for this registration")
            
            # Check if registration is eligible for payment
            if registration.status == 'cancelled':
                raise serializers.ValidationError("Cannot create payment for cancelled registration")
...
class PaymentCreateSerializer(PaymentSerializer):
    ...
    class Meta(PaymentSerializer.Meta):
        read_only_fields = [
            'id', 'user', 'user_detail', 'registration_detail', 'amount', 
            'currency', 'status', 'initiated_at', 'processed_at', 
...
class PaymentUpdateSerializer(PaymentSerializer):
    ...
    class Meta(PaymentSerializer.Meta):
        read_only_fields = [
            'id', 'registration', 'user', 'user_detail', 'registration_detail',
            'amount', 'currency', 'initiated_at', 'processed_at', 
            'completed_at', 'refunded_at'
        ]
